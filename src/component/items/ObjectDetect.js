...
const ObjectDetect = () => {
  ...
  const device = devices.find((d) => d.position === 'back'); // Use back camera only
  ...
  const processFrame = useCallback(async () => {
    ...
    if (frameCounter.current % 3 !== 0) return; // Skip 2 out of 3 frames
    ...
    if (detections.length > 0) {
      ...
      const hasChanged = currentDetections.some((curr, i) => {
        ...
        return (
          ...
          Math.abs(curr.confidence - prev.confidence) > 0.1
        );
      });

      if (hasChanged || currentDetections.length !== prevDetections.length) {
        const detectionText = detections.map((det) => `Detected ${det.label}`).join('. ');
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#FFFFFF', // Match index's detectionBox
  ...
  detectionBox: {
    ...
    borderColor: '#6C63FF', // Match index's purple theme
    borderRadius: moderateScale(4),
    backgroundColor: 'rgba(108, 99, 255, 0.1)', // Match index's icon container
  },
  label: {
    color: '#fff',
  ...
  errorContainer: {
    ...
    left: scale(20),
    right: scale(20),
    backgroundColor: 'rgba(108, 99, 255, 0.85)', // Match purple theme
  ...
  button: {
    ...
    backgroundColor: '#6C63FF', // Match index's purple
  ...
  message: {
    ...
    margin: scale(20),
    fontSize: moderateScale(16),
    color: '#6C63FF', // Match purple theme
    fontWeight: '500',
  },
  processingIndicator: {
    position: 'absolute',
